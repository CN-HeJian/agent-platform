...
**数据集自带脚本**，所以评测是确定性的：它评的是**平台行为**（给定这串模型输出，
工具该不该被调、观察该不该回填、预算该不该终止），不是某个模型的发挥——
一旦失败，你立刻知道是代码坏了，而不是"模型那天心情不好"。真实模型评测是另一件事：
同一个数据集换一个 adapter 就能跑，但那评的是"模型好不好"。

三级评估器，分级而不是给一个总分：

| 级别 | 看什么 | 能证明什么 |
|---|---|---|
| L1 规则 | 状态、工具调用集合、**禁止调用**、**失败原因**、步数 | 确定性断言。这一级红就是真坏 |
| L2 文本 | 最终答复是否包含期望片段 | 表述层面。它比 L1 脆（换个措辞就红），所以单独计 |
| L3 评审 | 交给另一个模型按评分标准判断 | 主观质量趋势。**没有评审模型时标 skipped，绝不算通过** |

`forbidTools` 与 `expectToolFailures` 是这套东西里最值钱的两列：
"期望它调什么"只能证明功能在，**"期望它绝不调什么 / 期望失败原因里出现这条规则"才能证明安全还在**。
后一列是做飞轮演示时发现的缺口——把危险命令检查摘掉之后，`expectTools` 照样满足
（工具照被调用）、`expectText` 照样满足（措辞一字没变），**数据集全绿**。
只有"结果里必须出现 `DESTRUCTIVE_RM`"才真的钉住了那道检查。

演示（真实跑过，记录见 DEMO-OUTPUT.md）：把 `CommandPolicy.check` 换成直接 allow，

```
  safe-command-blocked ❌  L1=FAIL
...
两处刻意不粉饰：**L3 没配评审模型时是 `skip` 而不是通过**（报告里单列条数）；
**基线里有、这次没跑的用例会被报出来**——删掉一条失败的用例不该让通过率变好看。

微调导出（U28）只导出**通过评测的**轨迹，并且把被排除的样本连同原因一起打出来
（而不是只给一个文件）。导出的是检查点里的完整消息序列，不是事件流——
事件流里有 token 分片与压缩记录，那些是平台的内部事实，不是模型该学的对话。
...
- **多实例下耐久与调度仍假设单实例**：`DurableRunner.reclaimOrphans()` 启动时把 `RUNNING`
  一律标成 `CRASHED`，`Scheduler.tick()` 每个副本都会扫一遍——多副本会让它们互相抢任务、
  同一条调度触发多次。需要"执行者 id + 心跳租约"与一把分布式锁。两处代码注释里都写着。
...
- **RBAC 已落地（U25），但密钥仍不能轮转**：角色授权、失败时关门、批准权与执行权分离都有了，
  **但没有密钥轮转**——换 key 要改 `APLAT_RBAC` 并重启。也没有"按用户"细到单个会话的授权，
  最细的粒度是角色。密钥只从环境变量读，不落代码、不落日志（统一走 `Secrets.redact`）。
...
- **工具表在装配后就固定了**：MCP（U24）与插件（U30）都是**启动时**挂载的，运行期新增的端点
  不会自动进工具表，要重启。前端同理，`GET /tools` 拿到的是装配期那一批。
...
- **`always` 的粒度是「会话 × 工具 × 任意参数」**：点一次「本会话总是允许」，对 `shell` 而言就是
  ...
  更细的粒度（按命令**模式**放行，比如只放行 `git status`）仍然没有——U25 给的是
  "角色 → 工具名"这一级，命令模式级要另做一层。
- **文档解析只覆盖"零依赖能做"的部分**：txt/md、csv、json、PDF（最小实现）。
  Office（docx/xlsx/pptx）、OCR、图片输入（U31 的"多模态"那一半）**都没做**——
  做它们要引重型库（POI / OCR），这一版刻意没付这个代价。
- **微调导出只出 ChatML 格式的 SFT 样本**：没有 DPO 偏好对、没有脱敏、没有数据集版本指纹
  （U28 测试要点里列了，但只做了"只导出通过评测的轨迹"这一半）。
- **K8s 清单没有灰度 / 蓝绿**（U33 的一部分）：只有 Deployment + ClusterIP + PVC。
  而且**没做过 `kubectl apply`**（本机没有集群），只做过静态校验。
